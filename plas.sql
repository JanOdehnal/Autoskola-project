...
SELECT distinct x.* , y.lector_id, z.* from timetable x left join student_course_lec y on x.student_id = y.student_id left join sides z on x.sides_id = z.id where x.lesson_date >= '2024-02-04' and (y.lector_id=1 or teacher_id =1) ORDER BY x.lesson_date, x.lesson_num;
